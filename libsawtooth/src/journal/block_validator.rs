...
impl BlockValidator {
    ...
    fn setup_thread(
        ...
        let state_validation =
            BatchesInBlockValidation::new(transaction_executor, merkle_state, scheduler_factory);
...
impl<SBV: BlockValidation<ReturnValue = BlockValidationResult>> BlockValidationProcessor<SBV> {
    ...
    fn validate_block(&self, block: &BlockPair) -> Result<BlockValidationResult, ValidationError> {
        ...
        for validation in &self.validations {
            match validation.validate_block(&block, &previous_blocks_state_hash) {
                Ok(()) => (),
        ...
        self.state_validation
            .validate_block(&block, &previous_blocks_state_hash)
    }
}
...
impl BlockValidation for DuplicatesAndDependenciesValidation {
    ...
    fn validate_block(&self, block: &BlockPair, _: &str) -> Result<(), ValidationError> {
        let batch_ids = block
            .block()
            .batches()
            .iter()
...
impl BlockValidation for PermissionValidation {
    ...
    fn validate_block(
        ...
        if block.header().block_num() != 0 {
            let state_root = hex::decode(prev_state_root).map_err(|err| {
                ValidationError::BlockValidationError(format!(
                "During permission check of block ({}, {}) state root was not able to be decoded: \
                    {}",
                block.block().header_signature(),
...
impl BlockValidation for OnChainRulesValidation {
    ...
    fn validate_block(
        ...
        if block.header().block_num() != 0 {
            let state_root = hex::decode(prev_state_root).map_err(|err| {
                ValidationError::BlockValidationError(format!(
                    "During validate_on_chain_rules, state root was not able to be decoded: \
                    {}",
                    err
                ))
            })?;
